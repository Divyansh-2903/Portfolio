...
// Portfolio-specific card data
const cardData: BentoCardProps[] = [
  {
    color: '#060010',
    title: 'Fullstack Development',
    description: 'React, Next.js, Node.js — from idea to deployment.',
    label: 'Dev'
  },
  {
    color: '#060010',
    title: 'Video Editing',
    description: 'Cinematic cuts, color grading, and storytelling.',
    label: 'Creative'
  },
  {
    color: '#060010',
    title: 'Motion Graphics',
    description: 'After Effects, GSAP & Framer Motion animations.',
    label: 'Motion'
  },
  {
    color: '#060010',
    title: 'UI/UX Design',
    description: 'Pixel-perfect interfaces grounded in user psychology.',
    label: 'Design'
  },
  {
    color: '#060010',
    title: 'Digital Marketing',
    description: 'SEO, analytics, and growth-driven strategies.',
    label: 'Growth'
  },
  {
    color: '#060010',
      ...
      <div className="bento-section w-full" ref={gridRef}>
        <div className="card-responsive grid gap-3">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-6 rounded-2xl border border-solid font-light overflow-hidden transition-all duration-300 hover:-translate-y-1 ${enableBorderGlow ? 'card--border-glow' : ''}`;
            const cardStyle = {
            ...
            return (
              ...
              >
                <div className="flex justify-between gap-3 relative text-white">
                  <span className="text-xs font-mono uppercase tracking-widest text-white/40">{card.label}</span>
                </div>
                <div className="flex flex-col relative text-white mt-auto">
                  <h3 className={`font-bold text-lg mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`}>{card.title}</h3>
                  <p className={`text-sm leading-relaxed opacity-60 ${textAutoHide ? 'text-clamp-2' : ''}`}>{card.description}</p>
                </div>
              </ParticleCard>
            );
          })}
